...
//parameter  check
localparam  c_WR_BYTE_WIDTH = c_WR_BYTE_EN ? c_A_DATA_WIDTH/(c_A_BE_WIDTH==0 ? 1 : c_A_BE_WIDTH) : ( (c_A_DATA_WIDTH%9) == 0 ? 9 : 8 );
...
localparam  N_DATA_1_WIDTH   =  (ADDR_WIDTH_WIDE <= 9) ? ((DATA_WIDTH_WIDE%9) == 0 ? 18 : 16) :        //cascade with 1k*18  type DRM 
                                (ADDR_WIDTH_WIDE == 10) ? ((DATA_WIDTH_WIDE%9) == 0 ? 9 : 8)   :        //cascade with 2k*9   type DRM 
...
localparam  L_DATA_1_WIDTH   =  (DATA_WIDTH_WIDE == 1)  ? 1:                                            //cascade with 16k*1  type DRM 
                                ...
                                (DATA_WIDTH_WIDE <= 9)  ? ((DATA_WIDTH_WIDE%9) == 0 ? 9 : 8)  :         //cascade with 4k*4   type DRM 
                                                          ((DATA_WIDTH_WIDE%9) == 0 ? 18 : 16);         //cascade with 1k*18  type DRM
//WIDTH_RATIO = 2
...
//    if (a_rst)
//        a_rd_en_ce <= 1'b0;
//    else if (c_A_WRITE_MODE != "NORMAL_WRITE")
//        a_rd_en_ce <= 1'b1;
//    else if (a_clk_en & ~a_addr_strobe)
//        a_rd_en_ce <= ~a_wr_en;
//end
//
//always @(posedge a_clk or posedge a_rst)
...
//    if (a_rst)
//        a_rd_en_oce <= 1'b0;
//    else if (a_clk_en & a_rd_oce)
//        a_rd_en_oce <= a_rd_en_ce;
//end
//
...
//        a_rd_en_invt <= 1'b0;
//    else if (a_clk_en & a_rd_oce)
//        a_rd_en_invt <= a_rd_en_ce;
//end
//
//assign a_rd_en_sel = (c_A_CLK_OR_POL_INV == 1) ? a_rd_en_invt : (c_A_OUTPUT_REG == 1) ? a_rd_en_oce : a_rd_en_ce;
...
begin
    if (a_rst)
        a_addr_bus_rd_ce <= 0;
    else if (c_A_WRITE_MODE != "NORMAL_WRITE") begin
    if (~a_addr_strobe & a_clk_en)
            a_addr_bus_rd_ce <= a_addr_bus[(ADDR_WIDTH_A-1): A_ADDR_SEL_LSB];
    end
    else if (~a_wr_en & ~a_addr_strobe & a_clk_en) begin
        a_addr_bus_rd_ce <= a_addr_bus[(ADDR_WIDTH_A-1): A_ADDR_SEL_LSB];
    end
end
...
begin
    if (a_rst)
        ...
        a_addr_bus_rd_oce <= a_addr_bus_rd_ce;
end
...
begin
    if (a_rst)
        ...
        a_addr_bus_rd_invt <= a_addr_bus_rd_ce;
end

assign  a_addr_bus_rd_sel = (c_A_CLK_OR_POL_INV == 1) ? a_addr_bus_rd_invt : (c_A_OUTPUT_REG == 1) ? a_addr_bus_rd_oce : a_addr_bus_rd_ce;
...
//    if (b_rst)
//        b_rd_en_ce <= 1'b0;
//    else if (c_B_WRITE_MODE != "NORMAL_WRITE")
//        b_rd_en_ce <= 1'b1;
//    else if (b_clk_en & ~b_addr_strobe)
//        b_rd_en_ce <= ~b_wr_en;
//end
//
//always @(posedge b_clk or posedge b_rst)
...
//    if (b_rst)
//        b_rd_en_oce <= 1'b0;
//    else if (b_clk_en & b_rd_oce)
//        b_rd_en_oce <= b_rd_en_ce;
//end
//
...
//        b_rd_en_invt <= 1'b0;
//    else if (b_clk_en & b_rd_oce)
//        b_rd_en_invt <= b_rd_en_ce;
//end
//
//assign b_rd_en_sel = (c_B_CLK_OR_POL_INV == 1) ? b_rd_en_invt : (c_B_OUTPUT_REG == 1) ? b_rd_en_oce : b_rd_en_ce;
...
begin
    if (b_rst)
        b_addr_bus_rd_ce <= 0;
    else if (c_B_WRITE_MODE != "NORMAL_WRITE") begin
    if (~b_addr_strobe & b_clk_en)
            b_addr_bus_rd_ce <= b_addr_bus[(ADDR_WIDTH_B-1): B_ADDR_SEL_LSB];
    end
    else if (~b_wr_en & ~b_addr_strobe & b_clk_en) begin
        b_addr_bus_rd_ce <= b_addr_bus[(ADDR_WIDTH_B-1): B_ADDR_SEL_LSB];
    end
end
...
begin
    if (b_rst)
        ...
        b_addr_bus_rd_oce <= b_addr_bus_rd_ce;
end
...
begin
    if (b_rst)
        ...
        b_addr_bus_rd_invt <= b_addr_bus_rd_ce;
end

assign  b_addr_bus_rd_sel = (c_B_CLK_OR_POL_INV == 1) ? b_addr_bus_rd_invt : (c_B_OUTPUT_REG == 1) ? b_addr_bus_rd_oce : b_addr_bus_rd_ce;
...
generate 
  for(gen_j=0;gen_j<ADDR_LOOP_NUM_A;gen_j=gen_j+1) begin:ADDR_LOOP 
     ...
     //write data 
        ...
        assign csa_mask_rd[gen_i + DATA_LOOP_NUM*gen_j] = ({a_cs_bit2_bus_m[gen_j],a_cs_bit1_m,a_cs_bit0_m} == csa_mask);// && (a_wr_en_bus[gen_i] == 1'b0)) ? 1'b1 : 1'b0;
        assign csb_mask_rd[gen_i + DATA_LOOP_NUM*gen_j] = ({b_cs_bit2_bus_m[gen_j],b_cs_bit1_m,b_cs_bit0_m} == csb_mask);// && (b_wr_en_bus[gen_i] == 1'b0)) ? 1'b1 : 1'b0;
